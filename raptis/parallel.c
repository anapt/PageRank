...
int main() {
    ...
    *************************************/
    ...
    #pragma omp parallel num_threads(nthreads)
      {
        #pragma omp single nowait
        {
            ...
            while (delta > options.tolerance && iter < options.maxiter){
                ...
                #pragma omp parallel for shared(sigma, x) num_threads(nthreads)
                for (i = 0; i < NUM_OF_NODES; i++){
                    sigma = 0;

                    // #pragma omp parallel for
                    // #pragma omp task 
                    #pragma omp parallel for shared(sigma) num_threads(nthreads)
                    // {
                        for (j=0; j < i-1; j++) {
                            sigma += K[i][j]*x[j];
                        }
    ...
    seq_time = (double)((endwtime.tv_usec - startwtime.tv_usec)/1.0e6
                        + endwtime.tv_sec - startwtime.tv_sec);
    printf("Parallel PageRank wall clock time = %f\n", seq_time);
